    ...
    def handle(self, year=None, *args, **options):
        ...
        for entity in entities:
            currency, created = Currency.objects.get_or_create(
                name=re.sub('\(.*\)', '',
                            entity.xpath('.//displayname/text()')[0]).strip(),
                iso=entity.attrib['type']
            )

            if created:
                self.stdout.write('Add currency "%s" (%s)'
                                  % (currency.name, currency.iso))

        # currency symbols
        doc_symbols = self._load_url('http://www.xe.com/symbols.php')
        entities = doc_symbols.xpath('.//table[@class="cSymbl_tbl"]'
            ...
            if sign:
                ...
                try:
                    ...
                    if not object.sign:
                        object.sign = sign
                        object.save()
                        self.stdout.write('Set sign for currency "%s"'
                                          % object.iso)
                except Currency.DoesNotExist:
                    self.stderr.write('Currency %s not found' % iso)

        # country
        doc_data = self._load_url('%s/supplemental/supplementalData.xml'
                                  % self._base_url)
        entities = doc_main.xpath('.//ldml/localedisplaynames'
                                  '/territories/territory[not(@alt)]')
        for entity in entities:
            iso = entity.attrib['type']
            ...
            if iso in self._ignore_regions:
                ...
                country_entity = doc_data.xpath('.//supplementaldata'
                                                '/codemappings/territorycodes'
                                                '[@type="%s"][@alpha3]'
                                                % iso)
                ...
                if country_entity:
                    ...
                    if created:
                        self.stdout.write('Add country "%s" (%s)'
        ...
        for entity in entities:
            ...
            if iso in self._ignore_regions:
                ...
                country_entity = doc_data.xpath('.//supplementaldata'
                                                '/codemappings/territorycodes'
                                                '[@type="%s"][@alpha3]'
                                                % entity.attrib['iso3166'])

                if country_entity:
                    currency_entities = entity.xpath('.//currency'
                                                     '[not(@tender="false")]')
                    ...
                    try:
                        ...
                        self.stderr.write('Not found country by iso "%s"'
                                          % country_iso)
                    else:
                        for currency_entity in currency_entities:
                            ...
                            try:
                                ...
                                # currency history
                                if not currency.countries\
                                   .filter(id=country.id).exists():
                                    ...
                                    if 'to' in currency_entity.attrib:
                                        history.date_to = datetime.strptime(
                                            currency_entity.attrib['to'],
                                            '%Y-%m-%d')

                                    if 'from' in currency_entity.attrib:
                                        history.date_from = datetime.strptime(
                                            currency_entity.attrib['from'],
                                            '%Y-%m-%d')

                                    history.save()

                                    self.stdout.write(
                                        'Import history currency "%s" [%s] '
                                        'country "%s" [%s]'
                                        % (currency.name, currency.iso,
                                           country.name, country.iso)
                                    )
                    ...
                    self.stderr.write('Not found region country by iso 2 "%s"'
                                      % entity.attrib['iso3166'])
