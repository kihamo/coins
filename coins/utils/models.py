...
class CoinImageFieldFile(ImageFieldFile):
    def _get_url(self):
        ...
        if hasattr(self.field, 'thumb_width') and hasattr(self.field, 'thumb_height')\
           and self.field.thumb_width > 0 and self.field.thumb_height > 0:
            url = '%s.%dx%d' % (url, self.field.thumb_width, self.field.thumb_height)

        if hasattr(self.field, 'thumb_format'):
...
class CoinImageField(ImageField):
    ...
    def __init__(self, verbose_name=None, name=None, width_field=None,
                 height_field=None, thumb_width=None, thumb_height=None,
                 thumb_format=None, **kwargs):
